...
namespace modbusHelper.Message
{
    ...
    internal class MessageGenerationModule
    {
        ...
        /// <summary>
        /// 读取数据请求报文 01 02 03 04
        /// </summary>
        /// <param name="slaveStation">从站地址</param>
        /// <param name="readMode">模式</param>
        /// <param name="starAdr">线圈地址</param>
        /// <param name="length">读取长度</param>
        /// <returns></returns>
        public static byte[] GetReadMessage(int slaveStation,ReadMode readMode,short startAdr,short length)
        {
            //定义临时字节列表
        ...
        // < returns ></ returns >
        public static byte[] GetArrayDataWriteMessage(int slaveStation, short startAdr, string value)
        {
            //定义报文临时存储字节集合
            List<byte> tempList = new List<byte>();

            byte[] tempValue = FormatConversion.strToToHexByte(value);
            ...
            //获取写入值的数量
            byte[] length = BitConverter.GetBytes(Convert.ToInt16(tempValue.Length/2 % 256));
            ...
            //获取写入值的字节数 并根据计算机大小端存储方式进行高低字节转换
            if (!BitConverter.IsLittleEndian)
            {
                Array.Reverse(tempValue);
            }
            byte count = Convert.ToByte(tempValue.Length);

            //插入写入值的字节数
            tempList.Add(count);

            //插入写入值字节集合
            tempList.AddRange(tempValue);
